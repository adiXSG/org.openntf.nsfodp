...
public abstract class AbstractEquinoxMojo extends AbstractMojo {
	...
	protected Optional<NSFODPContainer> initContainerIfNeeded(List<Path> updateSites, Path packageZip, Path customDominoConfig) {
		if(this.container) {
			Log log = getLog();
			...
			NSFODPContainer container = new NSFODPContainer(updateSites, packageZip, customDominoConfig, log, outputDirectory.toPath(), this.containerBaseImage);
			container.start();
			if(log.isInfoEnabled()) {
